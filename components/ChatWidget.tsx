...
const ChatWidget: React.FC = () => {
    ...
    useEffect(() => {
        if (isOpen && view === 'chat' && messages.length === 0) {
            setMessages([{ role: 'model', text: "Hello! I'm M A R T X AI. How can I help you today? (This is a UI demo)" }]);
        }
                ...
                return (
                    <div className="p-4 space-y-3">
                        <div className="text-center mb-4">
                            <h3 className="font-bold text-lg text-gray-900">M A R T X AI</h3>
                ...
                return (
                    <>
                        <div className="p-4 border-b border-gray-200">
                            ...
                            <h3 className="font-bold text-lg text-gray-900">Chat with M A R T X AI</h3>
                        </div>
                        <div className="p-4 flex-grow overflow-y-auto space-y-4">
                            {messages.map((msg, index) => (
                                <div key={index} className={`flex items-end gap-2 ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                                    {msg.role === 'model' && <ChatbotIcon className="h-6 w-6 bg-gray-200 text-black rounded-full p-1 flex-shrink-0"/>}
